...
function bloqueparigual_acf()
{
    if (function_exists('acf_register_block_type')) {
        acf_register_block_type([
            'name'              => 'bloqueparigual',
            'title'             => __('Bloque Par Igual', 'tictac'),
            'description'       => __('Bloque con dos columnas 50/50: carrousel izquierda y lista derecha', 'tictac'),
            'render_callback'   => 'bloqueparigual',
            'mode'              => 'edit',
            'icon'              => 'columns',
            'keywords'          => ['custom', 'parigual', '50/50', 'columnas', 'carrousel'],
            'supports'          => [
                'align' => false,
                'mode' => false,
                'jsx' => true
...
add_action('acf/init', 'bloqueparigual_acf');

// 2. Registrar los campos ACF
if (function_exists('acf_add_local_field_group')) :

    acf_add_local_field_group(array(
        'key' => 'grupo_bloqueparigual',
        'title' => 'Bloque Par Igual',
        'fields' => array(
            // BLOQUE IZQUIERDO
            array(
                'key' => 'field_parigual_tab_izq',
            ...
            array(
                'key' => 'field_parigual_etiqueta_izq',
                'label' => 'Etiqueta',
                'name' => 'parigual_etiqueta_izq',
                'type' => 'text',
                'required' => 0,
            ),
            array(
                'key' => 'field_parigual_titulo_izq',
                'label' => 'Título (H2)',
                'name' => 'parigual_titulo_izq',
                'type' => 'text',
                'required' => 1,
            ),
            array(
                'key' => 'field_parigual_parrafo_izq',
                'label' => 'Párrafo',
                'name' => 'parigual_parrafo_izq',
                'type' => 'wysiwyg',
                'required' => 0,
                'rows' => 3,
            ),
            array(
                'key' => 'field_parigual_repetidor_izq',
                'label' => 'Carrousel de Imágenes',
                'name' => 'parigual_repetidor_izq',
                'type' => 'repeater',
                'required' => 1,
                'min' => 1,
                'layout' => 'block',
            ...
            array(
                'key' => 'field_parigual_etiqueta_der',
                'label' => 'Etiqueta',
                'name' => 'parigual_etiqueta_der',
                'type' => 'text',
                'required' => 0,
            ),
            array(
                'key' => 'field_parigual_titulo_der',
                'label' => 'Título (H2)',
                'name' => 'parigual_titulo_der',
                'type' => 'text',
                'required' => 1,
            ),
            array(
                'key' => 'field_parigual_parrafo_der',
                'label' => 'Párrafo',
                'name' => 'parigual_parrafo_der',
                'type' => 'wysiwyg',
                'required' => 0,
                'rows' => 3,
            ),
            array(
                'key' => 'field_parigual_repetidor_der',
                'label' => 'Lista de Items',
                'name' => 'parigual_repetidor_der',
                'type' => 'repeater',
                'required' => 1,
                'min' => 1,
                'layout' => 'block',
            ...
            array(
                'key' => 'field_parigual_imagen_fondo',
                'label' => 'Imagen de Fondo del Bloque Derecho',
                'name' => 'parigual_imagen_fondo',
                'type' => 'image',
                'required' => 0,
                'return_format' => 'array',
                'instructions' => 'Imagen de fondo para el bloque derecho (verde)',
            ),
            array(
                'key' => 'field_parigual_boton_texto',
                'label' => 'Texto del Botón',
                'name' => 'parigual_boton_texto',
                'type' => 'text',
                'required' => 0,
                'default_value' => 'CONTACTAR',
            ),
            array(
                'key' => 'field_parigual_boton_icono',
                'label' => 'Icono del Botón',
                'name' => 'parigual_boton_icono',
                'type' => 'image',
                'required' => 0,
                'return_format' => 'array',
            ...
            array(
                'key' => 'field_parigual_boton_enlace',
                'label' => 'Página de Destino',
                'name' => 'parigual_boton_enlace',
                'type' => 'page_link',
                'required' => 0,
...
function bloqueparigual_scripts()
{
    if (!is_admin()) {
        wp_enqueue_style('bloqueparigual', get_stylesheet_directory_uri() . '/assets/functions/blocks/parigual/parigual.min.css');
    }
}
add_action('wp_enqueue_scripts', 'bloqueparigual_scripts');

// 4. Función de renderizado
function bloqueparigual($block)
{
    // Variables ACF para el bloque izquierdo
    $etiqueta_izq = get_field('parigual_etiqueta_izq');
    $titulo_izq = get_field('parigual_titulo_izq');
    $parrafo_izq = get_field('parigual_parrafo_izq');
    $repetidor_izq = get_field('parigual_repetidor_izq');

    // Variables ACF para el bloque derecho
    $etiqueta_der = get_field('parigual_etiqueta_der');
    $titulo_der = get_field('parigual_titulo_der');
    $parrafo_der = get_field('parigual_parrafo_der');
    $repetidor_der = get_field('parigual_repetidor_der');
    $imagen_fondo = get_field('parigual_imagen_fondo');
    $boton_texto = get_field('parigual_boton_texto');
    $boton_icono = get_field('parigual_boton_icono');
    $boton_enlace = get_field('parigual_boton_enlace');

    // Generar ID único para este bloque
    $block_id = 'bloqueparigual-' . uniqid();
?>

<section class="parigual-section <?php if (isset($block['className'])) { echo esc_attr($block['className']); } ?>" id="<?php echo $block_id; ?>">
    <div class="containerancho">
        <div class="parigual-wrapper">
            
            <!-- BLOQUE IZQUIERDO -->
            <div class="parigual-bloque parigual-izquierda">
                <?php if ($etiqueta_izq) : ?>
                    <span class="parigual-etiqueta"><?php echo esc_html($etiqueta_izq); ?></span>
                <?php endif; ?>

                <?php if ($titulo_izq) : ?>
                    <h2 class="parigual-titulo"><?php echo esc_html($titulo_izq); ?></h2>
                <?php endif; ?>

                <?php if ($parrafo_izq) : ?>
                    <p class="parigual-parrafo"><?= $parrafo_izq ?></p>
                <?php endif; ?>

                <?php if ($repetidor_izq && count($repetidor_izq) > 0) : ?>
                    <div class="parigual-carrousel">
                        <div class="carrousel-container">
                            <?php foreach ($repetidor_izq as $index => $item) : 
                                $imagen = $item['imagen'];
                            ...
                            ?>
                                <div class="carrousel-slide <?php echo $index === 0 ? 'active' : ''; ?>">
                                    <?php if ($imagen) : ?>
                                        <div class="carrousel-imagen-wrapper">
                                            <img src="<?php echo esc_url($imagen['url']); ?>" alt="<?php echo esc_attr($imagen['alt']); ?>">
                                            <div class="carrousel-overlay d-flex flex-row justify-content-between">
                                                <div class="carrousel-content">
                                                    <?php if ($titulo_item) : ?>
                                                        <p class="carrousel-titulo"><?php echo esc_html($titulo_item); ?></p>
                                                    <?php endif; ?>
                                                    <?php if ($texto_item) : ?>
                                                        <p class="carrousel-texto"><?php echo esc_html($texto_item); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                                
                                                <?php if (count($repetidor_izq) > 1) : ?>
                                                    <div class="carrousel-controls">
                                                        <button class="carrousel-prev" aria-label="Anterior">
...
<feBlend mode="normal" in="SourceGraphic" in2="effect1_dropShadow_2010_109" result="shape"/>
                                                        ...
                                                        <button class="carrousel-next" aria-label="Siguiente">
                                                            <svg width="21" height="27" viewBox="0 0 21 27" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<feBlend mode="normal" in="SourceGraphic" in2="effect1_dropShadow_2010_110" result="shape"/>
            ...
            <!-- BLOQUE DERECHO -->
            <div class="parigual-bloque parigual-derecha" <?php if ($imagen_fondo) : ?>style="background-image: url('<?php echo esc_url($imagen_fondo['url']); ?>');"<?php endif; ?>>
                <?php if ($etiqueta_der) : ?>
                    <span class="parigual-etiqueta"><?php echo esc_html($etiqueta_der); ?></span>
                <?php endif; ?>

                <?php if ($titulo_der) : ?>
                    <h2 class="parigual-titulo"><?php echo esc_html($titulo_der); ?></h2>
                <?php endif; ?>

                <?php if ($parrafo_der) : ?>
                    <p class="parigual-parrafo"><?= $parrafo_der ?></p>
                <?php endif; ?>

                <?php if ($repetidor_der && count($repetidor_der) > 0) : ?>
                    <div class="parigual-lista">
                ...
                <?php if ($boton_texto && $boton_enlace) : ?>
                    <div class="parigual-cta">
                        <a href="<?php echo esc_url($boton_enlace); ?>" class="btn-contactar">
...
(function() {
    ...
    const carrousel = bloque.querySelector('.parigual-carrousel');
    if (!carrousel) return;

    const slides = carrousel.querySelectorAll('.carrousel-slide');
    const allPrevBtns = carrousel.querySelectorAll('.carrousel-prev');
    const allNextBtns = carrousel.querySelectorAll('.carrousel-next');
    let currentSlide = 0;
    ...
    // Agregar evento a TODOS los botones prev
    allPrevBtns.forEach(btn => {
        btn.addEventListener('click', prevSlide);
    });

    // Agregar evento a TODOS los botones next
