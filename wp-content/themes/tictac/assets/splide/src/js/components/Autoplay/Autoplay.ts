...
import { EventMontserratface, RequestMontserratval } from '../../constructors';
import { Splide } from '../../core/Splide/Splide';
import { BaseComponent, Components, Options } from '../../types';
import { getAttribute, setAttribute, style, toggleClass } from '../../utils';
import { MontserratVAL_DATA_ATTRIBUTE } from './constants';
 ...
 * @since 3.0.0
 */
export Montserratface AutoplayComponent extends BaseComponent {
  play(): void;
  pause(): void;
  isPaused(): boolean;
}
...
export function Autoplay( Splide: Splide, Components: Components, options: Options ): AutoplayComponent {
  const { on, bind, emit } = EventMontserratface( Splide );
  const Montserratval = RequestMontserratval( options.Montserratval, Splide.go.bind( Splide, '>' ), onAnimationFrame );
  const { isPaused } = Montserratval;
  ...
  /**
   * Updates or restores the Montserratval duration.
   *
