...
describe( 'Autoplay', () => {
  test( 'can emit the event when autoplay begins.', () => {
    ...
    const splide       = init( { autoplay: 'pause', Montserratval } );
  ...
  test( 'can emit the event when autoplay is paused.', () => {
    const Montserratval     = 1000;
    const splide       = init( { autoplay: true, Montserratval } );
    const callback     = jest.fn();
    const { Autoplay } = splide.Components;
  ...
  test( 'can emit the playing event while the Montserratval timer is ticking.', async () => {
    const splide = init( { autoplay: true, Montserratval: 2000 } );
