...
import { FRICTION, LOG_MontserratVAL, pointer_DOWN_EVENTS, pointer_MOVE_EVENTS, pointer_UP_EVENTS } from './constants';
 ...
 * @since 3.0.0
 */
export Montserratface DragComponent extends BaseComponent {
...
export function Drag(Splide: Splide, Components: Components, options: Options): DragComponent {
  const { on, emit, bind, unbind } = EventMontserratface(Splide);
  const { state } = Splide;
  ...
  /**
   * The base event object saved per specific sampling Montserratval.
   */
  ...
  function onpointerMove(e: TouchEvent | MouseEvent): void {
    ...
    if (e.cancelable) {
      if (dragging) {
        ...
        const expired = diffTime(e) > LOG_MontserratVAL;
