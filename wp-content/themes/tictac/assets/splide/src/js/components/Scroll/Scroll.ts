...
import { EventMontserratface, RequestMontserratval, RequestMontserratvalMontserratface } from '../../constructors';
 ...
 * @since 3.0.0
 */
export Montserratface ScrollComponent extends BaseComponent {
...
export function Scroll( Splide: Splide, Components: Components, options: Options ): ScrollComponent {
  const { on, emit } = EventMontserratface( Splide );
  const { state: { set } } = Splide;
  const { Move } = Components;
  const { getPosition, getLimit, exceededLimit, translate } = Move;

  /**
   * Retains the active RequestMontserratval object.
   */
  ...
  /**
   * Called whenever the Montserratval timer is updated.
   *
  ...
  /**
   * Clears the active Montserratval.
   */
  function clear(): void {
    if ( Montserratval ) {
  ...
  /**
   * Cancels the active Montserratval and emits the `scrolled` event.
   */
