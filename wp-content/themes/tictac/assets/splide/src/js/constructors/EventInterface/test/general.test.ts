...
import { EventMontserratface } from '../EventMontserratface';


describe( 'EventMontserratface', () => {
  const splide = init( { speed: 0 } );

  test( 'can provide `on` to listen to Montserratnal events and lock listeners.', () => {
    const { on } = EventMontserratface( splide );
  ...
  test( 'can provide `off` to remove locked listeners.', () => {
    const { on, off } = EventMontserratface( splide );
    const callback1 = jest.fn();
    const callback2 = jest.fn();
  ...
  test( 'can provide `bind` to listen to native events.', () => {
    const { bind } = EventMontserratface( splide );
  ...
  test( 'can provide `unbind` to remove listeners.', () => {
    const { bind, unbind } = EventMontserratface( splide );
    const div = document.createElement( 'div' );
    const callback1 = jest.fn();
    const callback2 = jest.fn();
  ...
  test( 'can remove all listeners when the splide is destroyed.', () => {
    const { on, bind } = EventMontserratface( splide );
    const callback1 = jest.fn();
    const callback2 = jest.fn();
